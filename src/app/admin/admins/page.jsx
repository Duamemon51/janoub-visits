...
import { Plus, Edit, Trash2, X, User, Mail, Lock, ChevronLeft, ChevronRight } from "lucide-react";

// Fetcher with authentication handling
const fetcher = (url) =>
  fetch(url, {
...
export default function AdminsPage() {
  const { data: admins, mutate, error } = useSWR("/api/admin/admins", fetcher);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const url = isEditing ? `/api/admin/admins/${editId}` : "/api/admin/admins";
  ...
  if (error) return <div className="text-center text-red-500 font-semibold p-8">Failed to load admins. Please try again later.</div>;
  if (!admins) return <div className="flex justify-center items-center h-48"><div className="w-12 h-12 border-4 border-purple-200 border-t-purple-600 rounded-full animate-spin"></div></div>;
            ...
            <tbody>
              {currentAdmins.length > 0 ? (
                currentAdmins.map((admin) => (
                  <tr key={admin._id} className="border-b border-gray-100 last:border-b-0 hover:bg-gray-50 transition-colors">
                    <td className="p-4 text-gray-800 font-medium">{admin.name}</td>
          ...
          <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-lg transform scale-95 animate-scaleUp">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="relative">
                <label className="text-gray-600 text-sm font-medium sr-only">Name</label>
                <input
                  type="text"
                  placeholder="Name"
                  value={form.name}
