...
// -------------------- Interfaces --------------------
interface Place { _id: string; name: string; image?: string; type: "Place"; }
interface Category { _id: string; name: string; icon?: string; type: "Category"; }
interface Eat { _id: string; title: string; images?: string[]; location?: string; type: "Eat"; }
interface LiveEvent { _id: string; title: string; img: string; location: string; price: number; btn: string; dateFrom: string; dateTo: string; type: "LiveEvent"; }
interface Tour { _id: string; title: string; image?: string; description?: string; type: "Tour"; }
interface Destination { _id: string; title: string; img: string; tag?: string; location?: string; type: "Destination"; }

type SearchResult = Place | Category | Eat | LiveEvent | Tour | Destination;
...
const LiveEventCard = ({ event }: { event: LiveEvent }) => {
  ...
  const formattedDate = `${from.getDate().toString().padStart(2, "0")} ${from.toLocaleString(
    "en-US",
    { month: "short" }
  )} ${from.getFullYear()} → ${to.getDate().toString().padStart(2, "0")} ${to.toLocaleString(
    "en-US",
    { month: "short" }
  )} ${to.getFullYear()}`;
  ...
  return (
    <div className="w-[240px] rounded-lg overflow-hidden shadow relative">
      <div className="relative">
        ...
        <div
          className="absolute top-0 left-0 w-full text-white text-[11px] font-semibold px-3 py-2"
      ...
      <div className="bg-white p-3">
        ...
        <p className="text-[12px] text-gray-400 flex items-center gap-1">
          From {event.price}
          <img src="/cuurency.jpg" alt="SAR" className="w-3 h-3 object-contain" />
        </p>
    ...
    <div className="p-6 max-w-[1440px] mx-auto">
      ...
      {loading ? (
        ...
        <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {results.map((item) => {
            if (item.type === "Place") return <PlaceCard key={item._id} place={item} />;
            if (item.type === "Category") return <CategoryCard key={item._id} category={item} />;
            if (item.type === "Eat") return <EatCard key={item._id} eat={item} />;
            if (item.type === "LiveEvent") return <LiveEventCard key={item._id} event={item} />;
            if (item.type === "Tour") return <TourCard key={item._id} tour={item} />;
            if (item.type === "Destination") return <DestinationCard key={item._id} destination={item} />;
            return null;
          })}
        </div>
      )}
